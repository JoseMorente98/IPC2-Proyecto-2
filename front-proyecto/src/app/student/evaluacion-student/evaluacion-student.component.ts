...
@Component({
  selector: 'app-evaluacion-student',
  templateUrl: './evaluacion-student.component.html',
  styleUrls: ['./evaluacion-student.component.scss']
})
export class EvaluacionStudentComponent implements OnInit {
  ...
  goEvaluacion(id:number, data:any) {
    console.log(data)
    if(data=='1') {
      this.notificationsService.warn('Alerta D:', 'La evaluación se encuentre deshabilitada.');
    } else {
      this.router.navigate(['/estudiante/evaluacion/detail/'+id])
    }
  }
