...
@Component({
  selector: 'app-ev-det-assistant',
  templateUrl: './ev-det-assistant.component.html',
  styleUrls: ['./ev-det-assistant.component.scss']
})
export class EvDetAssistantComponent implements OnInit {
  formData:FormGroup;
  search:any;
  parameter:any;
  table:any[];
  ...
  getAll() {
    this.preguntaService.getByEvaluacion(+this.activatedRoute.snapshot.paramMap.get('id'))
    .subscribe((res) => {
      this.table = [];
      console.log(res);
      this.table = res;
